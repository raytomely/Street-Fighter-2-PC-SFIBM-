...
unsigned int loadImgSpr(char IdFn[],Image img[],Sprite spr[],int flag)
{
   ...
   fp1=fopen(getFilePath(IdFn),"r") ;
   if (fp1==NULL) cError(IdFn," Not Found !",1) ;

   fscanf(fp1,"%s",imgFn) ;
   fscanf(fp1,"%s",imgFn) ;
   fp2=fopen(getFilePath(imgFn),"rb") ;
...
void pPutRLESprMode(int x,int y,Sprite *spr,Image *img,int m)
{
   ...
   for ( i=0 ; i<spr->n ; i++ )
   {
     ...
     RLE_decoding(img[ip].p,_EBF,img[ip].ls,xs*ys);
     switch (m)
     {
       ...
       //default: blit_RLE(active_page, img[ip].p,img[ip].ls, x+spr->xo[i], y+spr->yo[i], xs, ys)    ; break ;
     }
   }
}
